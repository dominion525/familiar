...
- アクションは結果を**テキスト**で返します（終了ステータスではない）。読み取り系および複数失敗モードを持つ
  アクションは JSON エンベロープで返します:
  - `get_text` / `get_attribute` / `get_value` → `{"found": true, "value": "..."}` または `{"found": false}`
  - `select_option` / `submit` → `{"ok": true}` または `{"ok": false, "kind": "<理由>"}`
  - `exists` → 素の文字列 `"true"` / `"false"`（不在も `false` で表す）
  - その他の要素アクション（`click` / `fill` / `clear` / `set_checked` / `press_key` / `scroll_into_view`）
    → 成功時 `"true"`、要素未一致時 `"not_found"`
  - `query_all` → JSON 配列文字列（一致無しは `"[]"`）
...
wait_for_load WID TID N            readyState を最大 N 秒待つ → "complete" | "timeout"
...
```
get_text WID TID SEL               要素のトリム済みテキスト → {"found":true,"value":<text>} | {"found":false}
get_attribute WID TID SEL NAME     属性値（無ければ ""）→ {"found":true,"value":<val>} | {"found":false}
get_value WID TID SEL              input/textarea/select の値 → {"found":true,"value":<val>} | {"found":false}
...
select_option WID TID SEL VALUE    <option> を value/text で選択 → {"ok":true} | {"ok":false,"kind":"no_option"|"not_found"}
set_checked WID TID SEL BOOL       チェックの ON/OFF → "true" | "not_found"
press_key WID TID SEL KEY          合成 keydown/press/up → "true" | "not_found"
submit WID TID SEL                 要素のフォームを送信 → {"ok":true} | {"ok":false,"kind":"no_form"|"not_found"}
